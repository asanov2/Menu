...
import styles from './OwnerPushBlock.module.css';
...
export default function OwnerPushBlock() {
  ...
  const handleUnsubscribe = async () => {
    ...
    try {
      ...
      if (endpoint) {
        try { await unsubscribePush(endpoint); } catch { /* ignore */ }
      }
      setStatus('default');
  ...
  return (
    <div className={common.card}>
      <div className={styles.header}>
        ...
        <span className={styles.title}>Push-уведомления на этом устройстве</span>
      </div>
      ...
      {isIosNotStandalone() && status !== 'subscribed' && status !== 'unsupported' && (
        <div className={styles.hint}>
          <Icon name="info-circle" size={14} />
          <span>
            На iPhone push работают после установки: <strong>Поделиться</strong> →{' '}
      ...
      <div className={styles.footer}>
        Действует только на этом устройстве. На других устройствах включайте отдельно.
      </div>
    </div>
  );
}
