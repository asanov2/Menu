...
async def aggregate_yesterday() -> None:
    ...
    async with AsyncSessionLocal() as db:
        await _aggregate_date(db, yesterday)
...
async def _aggregate_date(db: AsyncSession, target_date: date) -> None:
    ...
    logger.info("aggregate_yesterday: processed %d restaurants for date=%s", count, target_date)
        ...
        text("""
            ...
            VALUES
                (gen_random_uuid(), :rid, :target_date, :menu_views, :item_views,
                 :top_items::jsonb, :device_breakdown::jsonb, :peak_hour, NOW(), NOW())
